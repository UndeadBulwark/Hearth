...
const api: BridgeAPI = {
  ...
  gameManager: {
    ...
    killGame: (installationId: string): Promise<boolean> => ipcRenderer.invoke(IPC_CHANNELS.GAME_MANAGER.KILL_GAME, installationId)
  },
  netManager: {
  ...
  dotnetManager: {
    getRequiredRuntime: (vsVersion: string): Promise<string | null> => ipcRenderer.invoke(IPC_CHANNELS.DOTNET_MANAGER.GET_REQUIRED_RUNTIME, vsVersion),
    isRuntimeCached: (runtimeId: string): Promise<boolean> => ipcRenderer.invoke(IPC_CHANNELS.DOTNET_MANAGER.IS_RUNTIME_CACHED, runtimeId),
    downloadRuntime: (runtimeId: string): Promise<boolean> => ipcRenderer.invoke(IPC_CHANNELS.DOTNET_MANAGER.DOWNLOAD_RUNTIME, runtimeId),
    getDotnetEnv: (runtimeId: string): Promise<Record<string, string> | null> => ipcRenderer.invoke(IPC_CHANNELS.DOTNET_MANAGER.GET_DOTNET_ENV, runtimeId),
    getRuntimeSize: (runtimeId: string): Promise<number> => ipcRenderer.invoke(IPC_CHANNELS.DOTNET_MANAGER.GET_RUNTIME_SIZE, runtimeId),
    onDownloadProgress: (callback: (event: Electron.IpcRendererEvent, runtimeId: string, progress: number) => void) => ipcRenderer.on(IPC_CHANNELS.DOTNET_MANAGER.DOWNLOAD_PROGRESS, callback)
  }
}
