...
ipcMain.handle(
  ...
  async (_event, version: GameVersionType, installation: InstallationType, accountId: string | null, dotnetEnv?: Record<string, string>): Promise<boolean> => {
    ...
    let command: string
    let params: string[]
    let env: NodeJS.ProcessEnv = { ...process.env, ...processEnv, ...dotnetEnv }
    ...
    if (accountId) {
      ...
      if (session) {
        const config = (await import("@src/config/configManager")).getConfig
        const configData = await config()
        const metadata = configData.accounts.find((a) => a.id === accountId)
        if (metadata) {
          account = {
            email: metadata.email,
            playerName: metadata.playerName,
            playerUid: metadata.playerUid,
            playerEntitlements: session.playerEntitlements,
            sessionKey: session.sessionKey,
            sessionSignature: session.sessionSignature,
            mptoken: session.mptoken,
            hostGameServer: session.hostGameServer
          }
          logMessage("info", `[back] [ipc] [ipc/handlers/gameHandlers.ts] [EXECUTE_GAME] Resolved account session for ${metadata.email}.`)
        } else {
          logMessage("warn", `[back] [ipc] [ipc/handlers/gameHandlers.ts] [EXECUTE_GAME] Account metadata not found for ${accountId}.`)
        }
      } else {
        logMessage("warn", `[back] [ipc] [ipc/handlers/gameHandlers.ts] [EXECUTE_GAME] Session not found for ${accountId}. Launching without account.`)
      }
    }
    ...
    if (command && params) {
      return new Promise((resolve, reject) => {
        ...
        externalApp.stderr.on("data", (data) => {
          lastStderrLine = data.toString().trim()
