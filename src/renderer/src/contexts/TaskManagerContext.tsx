...
export interface TaskType {
  ...
  name: string
  desc: string
  type: "download" | "extract" | "compress" | "runtime"
  progress: number
...
export const TaskProvider = ({ children }: { children: React.ReactNode }): JSX.Element => {
  ...
  useEffect(() => {
    ...
    window.api.utils.logMessage("info", `[front] [tasks] [contexts/TaskManagercontext.tsx] [TaskProvider] Adding listener for runtime download progress.`)
    window.api.dotnetManager.onDownloadProgress((_event, id, progress) => {
      if (progress === 100) return tasksDispatch({ type: ACTIONS.UPDATE_TASK, payload: { id, updates: { status: "completed" } } })
      tasksDispatch({ type: ACTIONS.UPDATE_TASK, payload: { id, updates: { progress, status: "in-progress" } } })
    })
  }, [])
  ...
  async function startRuntimeDownload(runtimeId: string, onFinish: (status: boolean, error: Error | null) => void): Promise<void> {
    ...
    try {
      ...
      tasksDispatch({ type: ACTIONS.UPDATE_TASK, payload: { id, updates: { status: "failed" } } })
      onFinish(false, new Error(`Error downloading runtime ${runtimeId}: ${err}`))
    }
  }
  ...
  return <TaskContext.Provider value={{ tasks, startDownload, startExtract, startCompress, startRuntimeDownload, removeTask }}>{children}</TaskContext.Provider>
}
