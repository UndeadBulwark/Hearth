    ...
    <Popover className="relative w-full">
      {({ open }) => (
        <>
          ...
          <AnimatePresence>
            {open && (
              <PopoverPanel static className="absolute bottom-full left-0 w-full mb-1 z-600 select-none rounded-sm overflow-hidden">
                <motion.ul
                  variants={DROPDOWN_MENU_WRAPPER_VARIANTS}
                  initial="initial"
                  animate="animate"
                  exit="exit"
                  className="max-h-60 flex flex-col bg-zinc-950/80 backdrop-blur-md border border-zinc-400/5 shadow-sm shadow-zinc-950/50 hover:shadow-none rounded-sm overflow-y-scroll text-sm"
                >
                  <AnimatePresence>
                    {tasks.length < 1 ? (
