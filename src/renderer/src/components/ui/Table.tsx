...
export function TableWrapper({ children, className }: { children: React.ReactNode; className?: string }): JSX.Element {
  return (
    <div className={clsx("rounded-sm bg-zinc-950/80 border border-zinc-400/5 shadow-sm shadow-zinc-950/50 hover:shadow-none duration-200 overflow-hidden backdrop-blur-md", className)}>{children}</div>
  )
}
...
export function TableHead({ children, className }: { children: React.ReactNode; className?: string }): JSX.Element {
  return <ul className={clsx("sticky top-0 z-10 bg-zinc-950/80 flex flex-col pr-[10px] backdrop-blur-md", className)}>{children}</ul>
}
