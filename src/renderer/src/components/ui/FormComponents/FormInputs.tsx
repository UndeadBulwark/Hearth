...
const INPUT_BASE_STYLES = `h-8 px-2 py-1 rounded-md placeholder:text-zinc-200/15 overflow-hidden outline-hidden backdrop-blur-md bg-zinc-950/80 border border-zinc-400/5`
...
export function FormToggle({ value, title, onChange }: { className?: string; value: boolean; title: string; onChange: (e: boolean) => void }): JSX.Element {
  return (
    <Switch
      ...
      onChange={onChange}
      title={title}
      className="shrink-0 group relative flex h-fit w-12 enabled:cursor-pointer backdrop-blur-md rounded-full border border-zinc-400/5 bg-zinc-950/80 p-1 enabled:shadow-sm enabled:shadow-zinc-950/50 enabled:hover:shadow-none transition-colors duration-100 ease-in-out focus:outline-hidden data-focus:outline-1 data-focus:outline-white data-checked:bg-vs"
    >
      <span
        aria-hidden="true"
