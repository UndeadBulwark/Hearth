    ...
    <div className="sticky top-0 z-50 w-full flex items-center justify-center">
      <div
        className={clsx(
          ...
          "before:absolute before:left-0 before:top-0 before:w-full before:h-full before:backdrop-blur-md",
          scrTop > 20 ? "bg-zinc-950/80" : "bg-zinc-950/80",
          className
        )}
      >
        {children}
        ...
        <span key={breadcrumb.name + breadcrumb.to} className="flex items-center">
          {index === 0 ? (
            <Link
              ...
              className="px-1 rounded-sm duration-200 hover:backdrop-blur-md hover:bg-zinc-950/80 border border-transparent hover:border-zinc-400/5"
            >
              <SlashSeparator />
            </Link>
          ) : (
