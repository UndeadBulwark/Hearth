...
function SocialButtons({ icon, to, text }: { icon: JSX.Element; to: string; text: string }): JSX.Element {
  return (
    <FormButton
      ...
      className={
        "text-lg backdrop-blur-md border border-zinc-400/5 bg-zinc-950/80 shadow-sm shadow-zinc-950/50 hover:shadow-none flex items-center justify-center gap-1 rounded-sm cursor-pointer px-1 duration-200"
      }
    >
      {icon}
      <span>{text}</span>
