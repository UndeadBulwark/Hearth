...
import { FormBody, FormFieldGroup, FormHead, FormLabel, FromGroup, FromWrapper, FormGroupWrapper, FormButton, FormInputText, FormToggle } from "@renderer/components/ui/FormComponents"
      ...
      <div className="min-h-full flex flex-col items-center justify-center gap-2">
        ...
        <FromWrapper className="max-w-[50rem] w-full my-auto">
          <FormGroupWrapper title={t("generic.general")}>
            ...
            <FromGroup className="items-center justify-between">
              <div className="flex-1 flex flex-col gap-0.5">
                <p className="font-bold text-sm">{t("features.config.hideLauncherWhilePlaying")}</p>
                <p className="text-zinc-400 text-xs">{t("features.config.hideLauncherWhilePlayingDesc")}</p>
              </div>
              <FormToggle
                title={t("features.config.hideLauncherWhilePlaying")}
                value={config.hideLauncherWhilePlaying}
                onChange={(v) => configDispatch({ type: CONFIG_ACTIONS.SET_HIDE_LAUNCHER_WHILE_PLAYING, payload: v })}
              />
            </FromGroup>

            <FromGroup className="items-center justify-between">
              <div className="flex-1 flex flex-col gap-0.5">
                <p className="font-bold text-sm">{t("features.config.showLauncherWhenGameStops")}</p>
                <p className="text-zinc-400 text-xs">{t("features.config.showLauncherWhenGameStopsDesc")}</p>
              </div>
              <FormToggle
                title={t("features.config.showLauncherWhenGameStops")}
                value={config.showLauncherWhenGameStops}
                onChange={(v) => configDispatch({ type: CONFIG_ACTIONS.SET_SHOW_LAUNCHER_WHEN_GAME_STOPS, payload: v })}
              />
            </FromGroup>
          </FormGroupWrapper>
      ...
      {({ open }) => (
        <>
          {SCALE_OPTIONS.filter((scale) => scale.key === selectedScale).map((scale) => (
            <ListboxButton
              key={scale.key}
              className="w-full h-8 px-2 py-1 flex items-center justify-between gap-2 rounded-sm overflow-hidden border border-zinc-400/5 bg-zinc-950/80 shadow-sm shadow-zinc-950/50 hover:shadow-none cursor-pointer"
            >
            ...
            {open && (
              <ListboxOptions static anchor="bottom" className="w-[var(--button-width)] z-600 mt-1 select-none rounded-sm overflow-hidden">
                <motion.ul
                  ...
                  animate="animate"
                  exit="exit"
                  className="flex flex-col bg-zinc-950/80 backdrop-blur-md border border-zinc-400/5 shadow-sm shadow-zinc-950/50 hover:shadow-none rounded-sm"
                >
