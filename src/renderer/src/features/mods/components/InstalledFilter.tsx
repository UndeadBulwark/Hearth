        ...
        <>
          {INSTALLED_FILTERS.filter((i) => i.key === installedFilter).map((lang) => (
            <ListboxButton
              key={lang.key}
              className={clsx(
                "px-2 py-1 flex items-center justify-between gap-2 rounded-sm overflow-hidden border border-zinc-400/5 bg-zinc-950/80 shadow-sm shadow-zinc-950/50 hover:shadow-none cursor-pointer",
                size
              )}
            >
